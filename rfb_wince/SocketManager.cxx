...
#include <rfb_wince/SocketManager.h>
...
void SocketManager::processEvent(HANDLE event) {
  if (listeners.count(event)) {
	...
	//
	// Thomas 23.05.2008 insert reinitialize the listener event 
	//
#if defined(_WIN32_WCE)
	  // Cancel event notification for this socket
	  WSAEventSelect(li.sock->getFd(), NULL, 0);

      WSAResetEvent(event);
	  // Reinitialize the event (without FD_ADDRESS_LIST_CHANGE !!!)
	  WSAEventSelect(li.sock->getFd(), event, FD_ACCEPT | FD_CLOSE);
	//////////
#endif
...
#else
	  if (WSAEventSelect(ci.sock->getFd(), NULL, 0) == SOCKET_ERROR)
#endif
