...
#include <rfb_wince/CompatibleBitmap.h>
#include <rfb_wince/BitmapInfo.h>
...
PixelFormat DeviceContext::getPF(HDC dc) {
...
#ifdef _WIN32_WCE
  // Peica Chen use the temporary method to get the BITMAPINFOHEADER
  // I just set it as BI_RGB,due to the a biCompression and RGBQUAD 
  // retrieve is more complex on the Windows CE.
  DIBSECTION dib;
  memset(&dib, 0, sizeof(dib));

  if (!::GetObject(bitmap,sizeof(DIBSECTION),(LPVOID)&dib)){
  ...
  bi.bmiHeader.biPlanes = dib.dsBm.bmPlanes;
  bi.bmiHeader.biBitCount = dib.dsBm.bmBitsPixel;
  // Set biCompression as BI_RGB
