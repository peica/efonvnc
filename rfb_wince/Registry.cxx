...
//  LONG result = RegCreateKey(root.key, name, &key);
  DWORD disposition;
  LONG result = RegCreateKeyEx(root.key,name,0,NULL,0,KEY_ALL_ACCESS,NULL,&key,&disposition);
  if (result != ERROR_SUCCESS) {
...
void RegKey::awaitChange(bool watchSubTree, DWORD filter, HANDLE event) const {
	// TODO: Implement the reg key changed notify
	// refer to http://geekswithblogs.net/BruceEitman/archive/2009/08/17/windows-ce-monitor-for-registry-changes.aspx
}
